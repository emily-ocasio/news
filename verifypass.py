    ...
    while True:
        ...
        for i, article in enumerate(articles):
            ...
            while len(answer) == 0 or not answer in "MONP":
                ...
                answer = input(f"Classify article as [M]assachusetts homicide, [O]ther place homicide, {extra}[N]ot homicide, [P]ass, [Q]uit for now > ")
                answer = answer.upper()
                if linesleft > 0 and answer=="X":
                    linesleft = print_wrapped(article['FullText'], start = 35, total = 200)
                if answer=="Q":
                    exit()
                if answer in "MNOP":
...
def verify_record(id, status):
    cur = db.cursor()
    cur.execute("UPDATE  verifications SET Status=? WHERE RecordId = ?", (status, id))
    db.commit()
    return
