...
from typing import cast, Callable, Generic, Self, TypeVar, ClassVar, Type, overload
...
class Tuple[L,A](Functor[A]):
    ...
    def __mul__(self: "Tuple[S, Callable[[B], C]]", other: "Tuple[S, B]") -> "Tuple[S, C]":
        """Overrides the * operator for use as apply."""
        return self._apply(other)

    def _apply(self: "Tuple[S, Callable[[B], C]]", other: "Tuple[S, B]") -> "Tuple[S, C]":
        """
        Applies the function wrapped in the Tuple to the second value.
        The Semigroup instance is used to combine the first values.
        """
        return self.__class__.make(self.fst.append(other.fst), self.snd(other.snd))

    def __rshift__(self: "Tuple[S, A]", m: Callable[[A], "Tuple[S, B]"]) -> "Tuple[S, B]":
        """Defines the right-hand side of the bind operation."""
        return self._bind(m)

    def _bind(self: "Tuple[S, A]", m: Callable[[A], "Tuple[S, B]"]) -> "Tuple[S, B]":
        """
    ...
    def __eq__(self, other) -> bool:
        ...
        return isinstance(other, Tuple) and self.fst == other.fst and self.snd == other.snd
...
class Threeple[L, A, B](Functor[A]):
    ...
    def __mul__(self: "Threeple[S, Callable[[C], D], B]", other: "Threeple[S, C, B]") -> "Threeple[S, D, B]":
        """Overrides the * operator for use as apply."""
        return self._apply(other)

    def _apply(self: "Threeple[S, Callable[[C], D], B]", other: "Threeple[S, C, B]") -> "Threeple[S, D, B]":
        """ Applies the function wrapped in the Threeple to the second value.
        The Semigroup instance is used to combine the first values.
        """
        return self.__class__.make(self.fst.append(other.fst), self.snd(other.snd), self.trd)
    ...
    def __eq__(self, other) -> bool:
        ...
        return isinstance(other, Threeple) and self.fst == other.fst and self.snd == other.snd and self.trd == other.trd
