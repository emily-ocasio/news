...
import pymonad.monad_og as monad_og
...
def kleisli_compose(
        function_f: Callable[[R], monad_og.Monad[S]], function_g: Callable[[S], monad_og.Monad[T]]
) -> Callable[[R], monad_og.Monad[T]]:
...
def monad_from_none_or_value(
        if_none: monad_og.Monad[T], if_value: Callable[[T], monad_og.Monad[T]], value: T
) -> monad_og.Monad[T]:
