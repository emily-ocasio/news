...
rename relationship relationship_raw
rename circumstance circumstance_raw

gen str244 weapon_key = lower(trim(itrim(weapon_raw)))
gen str244 relationship_key = lower(trim(itrim(relationship_raw)))
gen str244 circumstance_key = lower(trim(itrim(circumstance_raw)))
...
preserve
	...
	isid lookup_key
	keep lookup_key group_code
	rename lookup_key weapon_key
	rename group_code weapon
	save `weapon_lookup'
restore

merge m:1 weapon_key using `weapon_lookup', keep(master match) nogen
count if missing(weapon)
if r(N) > 0 {
	di as error "WARNING: assigning unmapped weapon values to fallback code 6"
	preserve
		keep if missing(weapon)
		keep weapon_raw
		duplicates drop
		sort weapon_raw
		list weapon_raw, clean noobs
	restore
	replace weapon = 6 if missing(weapon)
}
drop weapon_key
...
preserve
	...
	isid lookup_key
	keep lookup_key group_code
	rename lookup_key relationship_key
	rename group_code relationship
	save `relationship_lookup'
restore

merge m:1 relationship_key using `relationship_lookup', keep(master match) nogen
count if missing(relationship)
if r(N) > 0 {
	di as error "WARNING: assigning unmapped relationship values to fallback code 18"
	preserve
		keep if missing(relationship)
		keep relationship_raw
		duplicates drop
		sort relationship_raw
		list relationship_raw, clean noobs
	restore
	replace relationship = 18 if missing(relationship)
}
...
preserve
	...
	isid lookup_key
	keep lookup_key group_code
	rename lookup_key circumstance_key
	rename group_code circumstance
	save `circumstance_lookup'
restore

merge m:1 circumstance_key using `circumstance_lookup', keep(master match) nogen
count if missing(circumstance)
if r(N) > 0 {
	di as error "WARNING: assigning unmapped circumstance values to fallback code 19"
	preserve
		keep if missing(circumstance)
		keep circumstance_raw
		duplicates drop
		sort circumstance_raw
		list circumstance_raw, clean noobs
	restore
	replace circumstance = 19 if missing(circumstance)
}
drop circumstance_key

local strlist "cntyfips ori state agency agentype source solved month actiontype homicide situation vicsex vicrace vicethnic offsex offrace offethnic msa yearmonth vicrace_offrace"
...
*Labeling
label define agelabel 1 "<18" 2 "18-29" 3 "30-49" 4 "50-69" 5 "70+" 6 "n/a"
...
*Labeling
label  define weaponlabel 1 "Gun" 2 "Knife" 3 "Blunt object" 4 "Beating/strangulation" 5 "NEC" 6 "Unmapped/Other"
...
table (vicage_c) (vicsex vicrace_simple) if vicage_c<6 & vicsex<2,  ///
...
local controls_cir		i.weapon i.relationship i.circumstance i.agentype i.multiplevic i.multipleoff 

*Demographic + Circumstantial + Geography/Time
local controls_geo 		i.cntyfips  i.year
...
marginsplot, ///
	...
	xtitle(" ") ///
	ytitle(" ")
	graph save "`year_name'_female", replace
graph combine "`year_name'_male" "`year_name'_female"
graph display, xsize(10) ysize(5)
graph export "`year_name'.pdf", replace as(pdf)
graph close
